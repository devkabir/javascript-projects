                ...
                <ul className="list-group list-group-flush">
                  <li className="list-group-item">
                    ...
                    <span className="badge badge-info text-lg">
                      {country.currencies != undefined ? country.currencies[0].symbol : '-'}
                    </span>{" "}
                    {country.currencies != undefined ? country.currencies[0].name : '-'}
                  </li>
                  <li className="list-group-item">
...
country.getInitialProps = async (ctx) => {
  const res = await fetch(
    "https://restcountries.com/v2/all?fields=name,capital,currencies,flag,region,subregion,alpha2Code"
  );
